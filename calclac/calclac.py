...
def calculate_maximum_probability(mean: float, stddev: float, lines: int, text: str):
    ...
    cumulative_lengths = [0] + list(
        it.accumulate([len(t) for t in tokens])
    )  # add a zero at the start to represent the break before the text
    tokens = [
        "*"
    ] + tokens  # add a placeholder token at the start to represent the break before the text
    ...
    def get_p(i, j):
        ...
        p = normal_dist.cdf(letters + 0.5) - normal_dist.cdf(
            letters - 0.5
        )  # apply continuity correction to approximate a discrete distribution
        return p
    ...
    dp_table = (
        []
    )  # a dynamic programming table to store the probability of starting breakpoint i on line k
    path_table = (
        []
    )  # list of maximum-probability edges to each node for each path length
...
def calculate_total_probability(mean, stddev, lines, text):
    ...
    break_pattern = re.compile(r"[ \-]")
    cumulative_lengths = [0] + list(
        it.accumulate([len(t) for t in break_pattern.split(text)])
    )  # add a zero at the start to represent the break before the text
    # Define a function to return the probability of a line starting at index i (where i=0 represents the start of the text)
    # and ending at index j:
    def get_p(i, j):
...
def main():
    parser = argparse.ArgumentParser(
        description="Calculates the probability of a given number of lines containing a given text, assuming a normal distribution with specified parameters on the number of letters per line."
    )
    parser.add_argument(
        "--max",
        action="store_true",
        help="Return the maximum probability over all partitions (and the maximum-probability partition) instead of the total probability.",
    )
    parser.add_argument("mean", type=float, help="Mean of the normal distribution.")
    parser.add_argument(
        "stddev", type=float, help="Standard deviation of the normal distribution."
    )
    parser.add_argument("lines", type=int, help="Number of lacunose lines.")
    parser.add_argument(
        "text",
        type=str,
        help='The text (where spaces and hyphens indicate potential line breaks) to divide over the lines. This should be specified between quotation marks. (Example: "υ-περ ε-μου ι-να μοι δο-θη λο-γος εν α-νοι-ξει του στο-μα-τος μου εν παρ-ρη-σι-α γνω-ρι-σαι το μυ-στη-ρι-ον υ-περ ου πρε-σβευ-ω εν α-λυ-σει")',
    )
    ...
    if not args.max:
        ...
        print(
            "Probability of input text distributed over %d lines, assuming a normal distribution with mean %f and standard deviation %f: %.4E."
            % (lines, mean, stddev, p)
        )
    else:
        p, partition = calculate_maximum_probability(mean, stddev, lines, text)
        print(
            "Maximum probability of input text distributed over %d lines, assuming a normal distribution with mean %f and standard deviation %f: %.4E."
            % (lines, mean, stddev, p)
        )
